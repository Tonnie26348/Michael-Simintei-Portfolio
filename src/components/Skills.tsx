        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          {skillGroups.map((g, i) => (
            <motion.div
              ...
              className="bg-card border border-border rounded-xl p-5 sm:p-6 hover:border-primary/20 hover:shadow-md transition-all duration-300 group"
            >
              ...
              <div className="flex flex-wrap gap-2">
                {g.skills.map((s) => (
                  <span
                    key={s}
                    className="text-[11px] sm:text-xs font-medium px-2.5 sm:px-3 py-1.5 rounded-full bg-accent/60 text-accent-foreground border border-border hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors duration-200 cursor-default"
                  >
                    {s}
                  </span>
                ))}
